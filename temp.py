...
동작: --mode 로 fwd 또는 rev 를 **하나씩** 학습(50ep 고정, early-stop OFF=md).
      끝나면 그 방향의 history.json 에서 per-epoch test_bias → final·best_val·tail-5 출력.
      fwd·rev 둘 다 돌아 있으면 δ_obs=(rev−fwd)/2 를 세 규약으로 표시하고 md 와 대조.
산출: checkpoint/E1_smoke/dorga/{24to26,26to24}_split1/  (실 E1 미간섭).

실행(서버, 데이터·GPU 필요):
  python temp.py --mode fwd
  python temp.py --mode rev      # 둘 다 끝나면 δ 표 출력
"""
import argparse
import json
import sys
...
def main():
    ...
    ap.add_argument("--mode", required=True, choices=["fwd", "rev"])
    args = ap.parse_args()
    ...
    if all(both.values()):
        ...
        other = "rev" if args.mode == "fwd" else "fwd"
        print(f"\n  → 다른 방향도 돌리면 δ 표 출력:  python temp.py --mode {other}")
