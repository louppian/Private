...
백본만 교체해 rev(2026→2024) 을 split1~5 돌려 평균낸다. 비교 기준:
  md rev +0.004  ·  현재 Brixia 백본 E1 rev +0.224
5-split 평균 rev 이 +0.004 로 붙으면 백본이 원인 확정 → core.MRM_W 를 그걸로 확정.
"""
...
def run():
    ...
    for k in range(SPLITS):
        ...
        A.register(arm, E.cross_val_fold(ty, ey, k, folds=SPLITS))   # val fold k
    ...
    def _m(key):
        return sum(s[key] for s in per_split) / len(per_split)
    ...
    print("→ 평균이 +0.004 에 붙으면 백본이 원인. → core.MRM_W 를 그걸로 확정.")
    print("=" * 60)
