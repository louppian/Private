# -*- coding: utf-8 -*-
r"""
E5 — 반합성 음성 대조   [A1_검증실험계획 §3 E4 → 재번호 E5]
...
산출:  checkpoint/E5/ (가중치·npz) + Result/E5/ (per-run json) + Result/E5/e5_summary.json (누출곡선)
"""
...
from e4_positive_control import two_halves         # 동일 정합 half 분할 재사용
...
def run_frac(year, H1, H2, frac, reps, epochs, root):
    ...
    deltas, ss = [], []
    for isd in reps:
        fwd = A.run_arm(f"E5_{year}_frac{frac}_fwd",
                        degraded_splitter(year, H1, H2, 1.0, isd), isd, epochs, root)
        rev = A.run_arm(f"E5_{year}_frac{frac}_rev",
